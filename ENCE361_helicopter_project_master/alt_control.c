...
void increase_altitude(int8_t percent_change)
{
    ...
    int32_t setpoint = (current_alt _percent + percent_change);
    ...
    //send to motors
    set_main_PWM (control / 1000);//control is divided by 100 within PWM function so divide by 10?
...
void decrease_altitude(int8_t percent_change)
{
